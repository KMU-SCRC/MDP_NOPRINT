...
char ssid[]="dronedrone34";                               //공유기 SSID
char pass[]="12341234";                                   //공유기 비밀번호
char fixedTopic[100]="BIOLOGGER/PENGUIN/1/";              //고정 토픽
const char* mqtt_server="10.10.10.1";                     //MQTT 서버 주소
...
void loop(){
  ...
  if(!mqttClient.connected()){                            //MQTT 서버에 연결 안 되어있을시
    ...
    if(startSwitch){                                      //"START:"메세지 수신시 파일보내기 시작
      sendFile();
      while(mqttClient.connected()){                      //테스트용
        mqttClient.publish(fixedTopic,"EMPTY SD CARD");
        Serial.println("PUBLISHED MESSAGE : EMPTY SD CARD");
        delay(5000);
      }
      startSwitch=false;
    }
...
void sendMessage(String stringMessage){                   //String 타입을 MQTT 서버에 전송하고 출력하는 함수
  mqttClient.publish(fixedTopic,stringMessage.c_str());   //MQTT서버에 전송
...
//      sendMessage(message);                               //메세지 전송
      mqttClient.publish(fixedTopic,message.c_str());   //MQTT서버에 전송
    }
